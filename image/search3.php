...
$cariApa = (isset($_GET['find'])) ? $_GET['find'] : 'samsung'; # cari barang apa 
...
{
	$og_title = $results['items'][$kira]['pagemap']['metatags'][0]['og:title'];
	$og_url = $results['items'][$kira]['pagemap']['metatags'][0]['og:url'];
	$og_image = (isset($results['items'][$kira]['pagemap']['metatags'][0]['og:image'])) ? 
		$results['items'][$kira]['pagemap']['metatags'][0]['og:image']
		: '';
	$og_description = $results['items'][$kira]['pagemap']['metatags'][0]['og:description'];

	echo '<h1>' . $kira . ':' . $og_title . '</h1>';
	echo '<h6>' . $og_description . '</h6>';
	echo '<br><a target="blank" href="' . $og_url . '"></a>';
